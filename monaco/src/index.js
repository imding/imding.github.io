...
editor.setValue(`var n = 42;

n += #BEGIN_EDITABLE#21#END_EDITABLE#;
...
btnEditAnswers.onclick = () => {
    ...
    console.log(monaco.editor.tokenize(editor.getValue(), 'javascript'));
};

editor.onDidChangeCursorSelection(() => {
    //  if (showing markup)
    //  return
    //  else if (target span is opaque)
    //  disable read-only
    //  else
    //  enable read-only

    let msg = `line: ${editor.getPosition().lineNumber}, column: ${editor.getPosition().column}`;
