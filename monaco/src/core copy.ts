...
import { newTestJson, newStepJson, newMissionJson, newFileJson } from './components/JsonTemplates';
    ...
    subMenu(App.UI.pnlActions, [{
        ...
        items: {
            'assignment': {
                ...
                handler: () => switchStepType(stepType.text)
            },
            'code': {
                tipHeading: 'code',
                tip: 'code steps can contain editables where the learner can write code',
                handler: () => switchStepType(stepType.code)
            },
            'extension': {
                ...
                handler: () => switchStepType(stepType.interactive)
            }
        }
    }, {
...
function createMissionStructure(fromString?: string, override?: MissionJson) {
    ...
    debugGroup(['fromString', App.colors.code]).end(fromString);
    debugGroup(['override', App.colors.code]).end(override);
    ...
    debugGroup('Updated ', ['missionJson', code]).end({ missionJson });
    ...
    stepList = obj(missionJson.steps)
        ...
        .map((step, idx) => {
            debugGroup('Step ', [`${idx + 1}`, code], ' is ', [step.type, string]);

            if (step.type === stepType.code || step.type === stepType.interactive) {
                if (!step.model) {
                    step.model = {};
                    debug('Initialised ', [`stepList[${idx}].model`, code]);
                }
                ...
                obj(step.files).forEachEntry((fullName, file) => {
                    ...
                    debug([`stepList[${idx}].model["${fullName}"]`, code], ` is ${step.model[fullName] ? '' : 'not '}found, file is in `, [file.mode, string], ' mode');
                    ...
                    if (file.mode !== fileMode.noChange) {
                        ...
                        debug('Created new model for ', [fullName, string]);
                    }
                });
            }
            else {
    ...
    debugGroup('Updated ', ['stepList', code]).end(stepList);
...
function newStepListItem(placement?: number, override?: StepJsonOverride) {
    ...
    missionFiles.forEach(fullName => {
        ...
        debug('Updated ', [`stepList[${placement}].files`, App.colors.code], stepList[placement].files);
    });
...
function goToStep(stepNo: number) {
    ...
    debug(['activeStep', App.colors.code], ' is now ', [stepNo, App.colors.code]);
...
function loadStep(stepNo: number) {
    ...
    stepJson = stepList[stepNo - 1];
    debug('Fetched ', ['stepJson', code], ' from ', [`stepList[${stepNo - 1}]`, code], { stepJson });

    obj(stepCache).forEachKey(key => stepCache[key] = {});
    debug('Cleared ', ['stepCache', code]);
    ...
    if (App.UI.btnStepType.firstElementChild.innerText !== iconNames[stepJson.type]) {
        ...
        debug('Changed ', ['btnStepType', code], ' icon to ', [iconNames[stepJson.type], string]);
    }
...
function deleteStep(stepNo: number = activeStep) {
    ...
    if (stepList.length === 1) {
        ...
        debug([`stepList[${stepNo - 1}]`, App.colors.code], ' has been removed');
...
function switchStepType(toType: SingleType) {
    //  FIXME: make it work

    const { code, string } = App.colors;

    debugGroup('switchStepType(toType: "', [toType, string], '")');

    //  store current tab data to cache
    if (!stepJson.model) {
        debug('No model is associated with ', [`stepList[${activeStep - 1}]`, code]);
    }
    else {
        obj(stepJson.model).forEachEntry((fullName, model) => {
            ...
            debug([`stepJson.model["${fullName}"]`, code], ' is stored in ', [`stepCache["${cacheType}"]`, code], ' and deleted');
        });
    }

    //  handle step type transitions
    if (toType === stepType.text) {
    ...
    stepJson.type = stepType[toType];
    debug('Changed ', ['stepJson.type', code], ' to ', [stepType[toType], string]);

    App.UI.btnStepType.firstElementChild.innerText = iconNames[toType];
    debug('Changed ', ['btnSepType', code], ' icon to ', [iconNames[toType], string]);
...
function switchTab() {
    ...
    if (activeTab === event.target) {
        debug('Skipped loading the same tab');
    }
    else {
        storeAnswers();
...
function loadTab(tab: Tab = activeTab) {
    ...
    if (stepJson.files[tab.innerText].contents) {
        debug([tab.innerText, string], ' has contents, using ', [`stepJson.model["${tab.innerText}"]`, code]);
        ...
        if (App.UI.btnCodeMode.firstElementChild.innerText !== iconNames[stepJson.files[tab.innerText].mode]) {
            ...
            debug('Changed ', ['btnCodeMode', code], ' icon to ', [iconNames[stepJson.files[tab.innerText].mode], string]);
        }
    }
    else {
        debug([`stepJson.files["${tab.innerText}"]`, code], ' has no content, resolving...');
        ...
        debug('Changed ', ['btnCodeMode', code], ' icon to ', [iconNames[fileMode.noChange], string]);
    }
    ...
    if (codeEditor) {
        ...
        debug('Updated ', ['codeEditor', code], ' model to ', [`stepJson.model["${tab.innerText}"]`, code]);
    }
    else if (diffEditor && stepJson.type === stepType.interactive) {
        switchDiffToCode();
        debug('Reverted to "code authoring mode" because "edit answers" is not compatible with ', [stepJson.type, string], ' steps');
    }
        ...
        onFail: () => {
            storeAnswers();
            switchDiffToCode();
            debug('Reverted to "code authoring mode" because the ', [tab.innerText, string], ' tab doesn\'t contain editable markup');
        },
    });
...
function resolveTabContent(cutoffStep: number, tabName: string): monaco.editor.IModel {
    ...
    debugGroup('resolveTabContent(cutoffStep: ', [`${cutoffStep}`, code], ', tabName: ', [tabName, string], ')');
    ...
    for (let i = cutoffStep - 1; i >= 0; i--) {
        ...
        if (step.type !== stepType.code) {
            ...
            debug([tabName, string], ' in step ', [`${i + 1}`, code], ' is ', [step.type, string], ', backtracking...');
        }
        else if (step.files) {
            if (step.files[tabName].mode === fileMode.newContents) {
                debug([tabName, string], ' in step ', [`${i + 1}`, code], ' is in ', [fileMode.newContents, string], ' mode');
                i = 0;
            }
            else {
                debug([tabName, string], ' in step ', [`${i + 1}`, code], ' is in ', [step.files[tabName].mode, string], ' mode, backtracking...');
            }
        }
        else {
            alert('fix me');
    ...
    debugGroup('Iterating on ', ['stepChain', code], ' to determine ', [tabName, string], ' content in step ', [`${activeStep}`, code]);

    const firstStep = stepChain.shift();
    const authorContent = firstStep.model[tabName].getValue();
    const authorEditableContents = authorContent.match(editablePattern.excludingMarkup);

    let transformedContent: string =
        authorEditableContents === null ?
            authorContent :
            getCodeWithAnswers(firstStep, tabName, authorContent, authorEditableContents);
    ...
    const resolved = stepChain.every((step, idx) => {
        ...
        if (step.type !== stepType.code) {
            debug('Step ', [stepNo, code], ' is ', [step.type, string], ', skipping...');
            return true;
        }
        if (step.files[tabName].mode === fileMode.noChange) {
            const _authorContent = step.model[tabName].getValue();
            const _authorEditableContents = _authorContent.match(editablePattern.excludingMarkup);

            transformedContent =
                _authorEditableContents === null ?
                    _authorContent :
                    getCodeWithAnswers(step, tabName, _authorContent, _authorEditableContents);

            debugGroup('Step ', [stepNo, code], ' is ', [fileMode.noChange, string], ', resolved').end(transformedContent);
            return true;
        }
...
function getCodeWithAnswers(step: StepJson, tabName: string, authorContent: string, authorEditableContents: Array<string>) {
    // debugGroup('getCodeWithAnswers()');
    
    const storedAnswers = step.files[tabName].answers || [];
    ...
    if (storedAnswers.length !== authorEditableContents.length) {
        if (step.files[tabName].mode === fileMode.noChange) {
            // debug('Resolving model answers...');

            const prevSteps = Array.from(stepList).slice(0, activeStep - 1).reverse();

            prevSteps.some((step, idx) => {
                if (step.type !== stepType.code) return;
                // debug('Found usable model answers in step', activeStep - idx - 1);
                return step.files[tabName].answers.map((answer: string, i: number) => storedAnswers[i] = answer);
            });
        }
        else {
            //  clear the answers array if the number of editables don't match with the number of answers
            step.files[tabName].answers = [];
            // debug('Cleared ', [`step.files["${tabName}"].answers`, App.colors.code]);
        }
    }
    ...
    authorEditableContents
        ...
        .forEach((content: string, idx: number) => {
            if (storedAnswers[idx] === undefined) {
                codeChunks[idx] += content.trim().length ? content.trim() : content;
                // debug('Editable', idx, `using author editable content "${content}"`);
            }
            else {
                codeChunks[idx] += storedAnswers[idx].trim();
                // debug('Editable', idx, `using stored answer "${storedAnswers[idx].trim()}"`);
            }
        });
...
function switchFileMode(mode: SingleMode) {
    if (mode !== fileMode.newContents && activeStep === 1) {
        return debug('File contents must be manually defined for the first step');
    }
    else if (stepJson.type !== stepType.code) {
        return debug('The ', [mode, App.colors.string], ' option is not compatible with ', [stepJson.type, App.colors.string], ' steps');
    }
    else if (diffEditor) {
        return debug('Disable "edit answer" mode before changing file mode');
    }
    ...
    const cacheCurrentModel = () => {
        ...
        debugGroup('Stored content in ', [`stepCache.${stepJson.files[activeTab.innerText].mode}["${activeTab.innerText}"]`, code]).end(codeEditor.getValue());
    };
    ...
    if (mode === fileMode.noChange) {
        ...
        if (stepCache[mode][activeTab.innerText]) {
            debug('Switching to ', [mode, string], ' with cache');

            stepJson.model[activeTab.innerText] = stepCache[mode][activeTab.innerText];
            debug([`stepJson.model["${activeTab.innerText}"]`, code], ' now using ', [`stepCache.${mode}["${activeTab.innerText}"]`, code]);
        }
        else {
            debug('Switching to ', [mode, string], ' without cache');
            ...
            if (mode === fileMode.newContents) {
                ...
                debug([`stepJson.model["${activeTab.innerText}"]`, code], ' now using the default ', [activeTab.type.toUpperCase()], ' model');
            }
            else {
                stepJson.model[activeTab.innerText] = monaco.editor.createModel(codeTemplate.transition, langType.js);
                debug([`stepJson.model["${activeTab.innerText}"]`, code], ' now using the default "transition" model');
            }
        }
    ...
    debug([`stepJson.model["${activeTab.innerText}"]`, code], ' is applied to the code editor');
...
function updateDiffEditor(cfg: DiffEditorConfig) {
    ...
    if (stepJson.type !== stepType.code) {
        return debug(['"edit answer mode"', string], ' is not compatible with ', [stepJson.type, string], ' steps');
    }
    ...
    const authorContent = original.getValue();
    ...
    const modified = monaco.editor.createModel(
        getCodeWithAnswers(stepJson, tab.innerText, authorContent, authorEditableContents),
        langType[tab.type]
    );
...
function storeAnswers() {
    ...
    if (answers) {
        ...
        debugGroup('Updated ', [`stepJson.files["${activeTab.innerText}"].answers`, App.colors.code]).end(stepJson.files[activeTab.innerText].answers);
    }
    else debug(`No answer found in "${activeTab.innerText}"`);
...
function parseDebug(segments: RichText) {
...
function debug(...segments: RichText) {
    if (!consoleDebug) return;
    console.log(...parseDebug(segments));
}

function debugGroup(...segments: RichText) {
    console[`group${consoleFold ? 'Collapsed' : ''}`](...parseDebug(segments));
