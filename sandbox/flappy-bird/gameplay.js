...
App.Main.prototype = {
	...
	create: () => {
		...
		for (let i = 0; i < this.GA.max_units; i++) {
	...
	update: () => {
		switch (this.state) {
			...
			case this.STATE_START: // start/restart the game
				...
				// reset barriers
				this.BarrierGroup.forEach(barrier => {
			...
			case this.STATE_PLAY: // play Flappy Bird game by using genetic algorithm AI
				...
				this.BirdGroup.forEachAlive(bird => {
...
var TreeGroup = (game, parent, index) => {
...
TreeGroup.prototype.getWorldX = () => {
	return this.topTree.world.x;
};

TreeGroup.prototype.getGapX = () => {
	return this.bottomTree.world.x + this.bottomTree.width;
};

TreeGroup.prototype.getGapY = () => {
...
Bird.prototype.restart = (iteration) => {
...
var Text = (game, x, y, text, align, font) => {
