...
let sampleBaseUrl = 'storage';//'https://s3-us-west-2.amazonaws.com/s.cdpn.io/969699';
...
Promise.all([
    ...
    then(([vars]) => {
        ...
        function toNoteSequence(pattern) {
            var notes = _.flatMap(pattern, (step, index) =>
                step.map(d => ({
                    pitch: midiDrums[d],
                    startTime: index * 0.5,
                    endTime: (index + 1) * 0.5
        ...
        WebMidi.enable(err => {
            ...
            function onDevicesChanged() {
                ...
                internalOption.id = "internalOption";
        ...
        document.querySelector('#temperature').addEventListener('input', evt => temperature = +evt.target.value);
