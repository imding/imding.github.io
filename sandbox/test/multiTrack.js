...
btnAddPath.onclick = addPathBranch;
...
function addPathBranch() {
    var roads = gridCells.flat().filter(cell => cell.id != 'finish' && cell.className.includes('road'));
    var randomRoadIndex = Math.floor(Math.random() * roads.length);
    var prevRoad = roads[randomRoadIndex];
    var newRoad;
    
    do {
        prevRoad.paths = prevRoad.paths.filter(cell => cell.className == '');

        var randomBranchIndex = Math.floor(Math.random() * prevRoad.paths.length);

        newRoad = prevRoad.paths[randomBranchIndex];
        newRoad.paths = newRoad.paths.filter(cell => cell != prevRoad);

        if (newRoad.offsetTop > prevRoad.offsetTop) {
            prevRoad.outDir.push('down');
            newRoad.inDir.push('up');
        }
        else if (newRoad.offsetTop < prevRoad.offsetTop) {
            prevRoad.outDir.push('up');
            newRoad.inDir.push('down');
        }
        else {
            prevRoad.outDir.push('right');
            newRoad.inDir.push('left');
        }

        //  assign appropriate road type image
        var roadType = prevRoad.inDir.concat(prevRoad.outDir).sort().join('');

        prevRoad.setRoadType(roadType);

        prevRoad = newRoad;
    }
    while (newRoad.className == '');
}

function plotFirstPath() {
    var randomRowIndex = Math.floor(Math.random() * gridCells.length);

    car.currentRow = gridCells[randomRowIndex];

    var prevRoad = car.currentRow[0];
    var newRoad;

    prevRoad.inDir.push('left');
