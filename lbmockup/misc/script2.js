...
let verdict, pass, inputClone = input, ruleCount = 0, tree = {atRules: [], rules: []};
...
function buildInvalidCode(nextValid = [pRule, pNestedAtRule, pAtRule], invalidCode = '') {
    ...
    // get position before inputClone.trim()
    ...
    if (/{[^{}]+}/.test(invalidCode)) {
        verdict = `The ${(ln.nth + 1).rank()} rule in your CSS has no selector.`;
    }
...
function parse() {
    while (inputClone.trim().length && !verdict) {
        ...
        if (tree.rules.length + tree.atRules.length) {
            ...
            pass = checkProperty(d) && checkValue(d);
            break;
        }
    }
}
...
function checkAtRule(match, nested) {
    ...
    if (nested) {
        const
            ...
            {
                error: !match[2],
                feedback: `There should be a space between @${match[1]} and ${match[3].trim()}.`,
            }];
    ...
    // @import, @charset or @namespace
        ...
        {
            error: !match[1],
            feedback: 'Please write the type of at-rule after the @ symbol.',
        },
        {
            error: !/charset|import|namespace/.test(match[1]),
            feedback: `@${match[1]} is not a valid type of at-rule.`,
        },
        {
            error: /[A-Z]/.test(match[1]),
            feedback: `@${match[1]} is incorrect. Make sure all letters are lowercase.`,
        },
        {
            ...
            feedback: function() {
                ...
                return `The @${match[1]} at-rule needs a ${word}.`;
            }(),
        },
        {
            error: !match[2],
            feedback: `There should be a space between @${match[1]} and ${match[3]}.`,
        },
        {
            error: !match[4],
            feedback: `Don't forget the semi-colon(;) after ${match[3]}`,
        }];
...
function checkRule(match, nested) {
    const
        ...
        {
            error: sel.some(s => /^[.#]/.test(s) && !/[a-z]/.test(s[1])),
...
btnParse.onclick = function() {
    ...
    console.log(t);// console.log(tree);
