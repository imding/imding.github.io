...
describe('CssAstTest', function () {
  ...
  it('fail declarations when declarationsOnly is false', () => {
    const css = `color: red;`;
    const message = [{ error: 'color: red; is incorrect. Please read the instructions again.' }];
    verify(css, message, { declarationsOnly: false });
  });

  it('pass declarations when declarationsOnly is true', () => {
    const css = `color: green;`;
    verify(css, [], { declarationsOnly: true });
  });
});
